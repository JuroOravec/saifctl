...
learning_outcomes:
  ...
  - '`feature.yml` configures phase defaults (critics, mutability, the v1 group blocks — see next bullet); `phase.yml` overrides per-phase.'
  - 'Per-phase config groups: `gate.*` (script + retries), `agent.*` (profile, install, env, model, base-url, secrets, reviewer), `container.*` (startup, cedar, no-leash, image, sandbox-profile, engine, compose-file), `runner.*` (test profile/image/script, stage script, resolve-ambiguity, test-retries), `limits.*` (max-attempts), and the `tests.none` declaration for phases that have no own tests.'
