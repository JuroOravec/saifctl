...
learning_outcomes:
  ...
  - 'Phase boundaries may trigger a controlled coder-container restart when adjacent phases differ on Level-2 settings (`agent.profile`, `agent.install`, `container.startup` / `cedar` / `no-leash`) or Level-3 settings (`container.image` / `sandbox-profile` / `engine` / `compose-file`). The sandbox dir survives; only the container is rebuilt. See `concept per-phase-config` for the lifecycle levels.'
