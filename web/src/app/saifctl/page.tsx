...
// URLs
const GITHUB_DOCS_SECURITY = '/product/saifctl';
const LEASH_URL = 'https://github.com/strongdm/leash';
const CEDAR_POLICY_URL = 'https://www.cedarpolicy.com/';
const VSCODE_EXTENSION_URL = VSCODE_MARKETPLACE_URL;
const VSCODE_EXTENSION_DOCS_URL = '#vscode-docs'; // TODO
...
const ProveItCTA = () => {
  return (
    <div
      className="p-8 mx-auto mt-20 text-left rounded-xl max-w-2xl"
      style={{ border: '1px solid var(--c-link-glow)', backgroundColor: 'var(--c-link-dim)' }}
    >
      <h3 className="text-fg font-bold text-lg mb-2">Want to see the Gauntlet in action?</h3>
      ...
      <div className="bg-surface border border-border rounded-md p-4 font-mono text-xs text-accent space-y-1 mb-6">
        <div>
          <span className="text-fg-subtle">{'// '}</span>Dropping in a few weeks.
        </div>
        <div>
          <span className="text-fg-subtle">{'// '}</span>The Docker sandboxes and VSCode extension
          are baking.
        </div>
      </div>
      <a
...
export default function SaifctlPage() {
      ...
      <main className="pt-24 pb-24">
        ...
        <section className="max-w-6xl mx-auto px-6 mb-28">
          ...
          >
            ...
            */}
            {/* # TODO - Mention real-time results and human feedback */}
            {/* # TODO - Mention full visibility into the entire infra (all containers, networks, etc.) */}
            {/* # TODO - Distributed Orchestration (planned, out of scope for alpha; Hatchet) */}
            {/* # TODO - Kubernetes support (planned, out of scope for alpha) */}
            {/* # TODO - Ext works across VSCode, Cursor, Antigravity, etc. */}
            {/* # TODO - See https://www.youtube.com/watch?v=pzUn9wTCgcw  3-9 mins in. The idea - instead of requiring users to write lengthy specs, use an agent to pierce through a POC to discover the edge cases and work's scope dynamically. The argument is that writing a dirty POC is 90% of time time the correct way to discover the requiremetns, because humans (and agents) are just bad / impossible at predicting.
                         -> Share with Theo
                         -> Share with Linear
            */}
            ...
            <div className="text-lg md:text-xl text-fg-muted mb-10 max-w-2xl mx-auto leading-relaxed space-y-4">
              <p>
        ...
        <section id="gauntlet" className="bg-surface border-y border-border py-24">
          <div className="max-w-6xl mx-auto px-6">
            <SectionLabel label="The Architecture" />
            <h2 className="text-3xl md:text-4xl font-bold text-fg mb-6 max-w-3xl">
              Code doesn't leave the sandbox until it survives the Gauntlet.
            </h2>
            <p className="text-fg-muted max-w-2xl mb-16 leading-relaxed">
              Most AI coding tools put you in the loop. You prompt, you review, you fix, you prompt
              again. <strong className="text-fg">You are the quality gate.</strong>
              <br />
              <br />
              ...
              <span className="font-bold text-fg">your own rules</span> - linters, type-checkers,
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        style={
                          isSelected
                            ? { borderColor: 'var(--c-link)', backgroundColor: 'var(--c-link-dim)' }
                            : {}
                        }
                        className={`flex flex-col items-center gap-2 w-full sm:w-28 sm:h-full py-2 rounded-xl transition-all cursor-pointer text-left border-2 border-transparent hover:border-link ${
                          isSelected ? '' : 'hover:bg-surface-2/50'
                        }`}
                      >
                        <div
                          style={
                            isSelected
                              ? {
                                  backgroundColor: 'var(--c-link-dim)',
                                  borderColor: 'var(--c-link)',
                                }
                              : {}
                          }
                          className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl shrink-0 transition-colors ${
                            isSelected ? 'border' : 'bg-surface-2 border border-border'
                          }`}
                        >
                          {node.icon}
                        </div>
                        <span className="font-bold text-fg text-xl sm:text-sm text-center">
                          {node.title}
                        </span>
                        <span className="text-fg-subtle text-lg sm:text-xs text-center leading-snug sm:w-28 px-1">
                          {node.desc}
                        </span>
                        <div className="text-fg-muted text-sm leading-relaxed text-left px-4 sm:hidden">
                          {node.detail}
                        </div>
                      </button>
                      {i < PIPELINE_NODES.length - 1 && (
                        <div className="flex flex-col sm:flex-row items-center sm:mx-1 my-2 sm:my-0 sm:mb-10 sm:self-center">
                          <div className="w-px h-6 sm:w-6 sm:h-px bg-border" />
                          <ChevronDown className="w-4 h-4 text-fg-subtle sm:hidden" />
                          <ChevronRight className="w-4 h-4 text-fg-subtle -ml-1 hidden sm:block" />
                        </div>
                      )}
                    </div>
                  );
            ...
            <div
              className="hidden sm:block mt-8 p-6 rounded-xl max-w-3xl"
              style={{ border: '1px solid var(--c-link)', backgroundColor: 'var(--c-link-dim)' }}
            >
            ...
            <div
              className="mt-12 p-6 rounded-xl max-w-3xl"
              style={{
                border: '1px solid var(--c-link-glow)',
                backgroundColor: 'var(--c-link-dim)',
              }}
            >
              <p className="text-fg-muted text-sm">
                <strong className="text-fg">Every artifact is inspectable.</strong>{' '}
                <code className="text-accent">proposal.md</code> →{' '}
                <code className="text-accent">specification.md</code> →{' '}
                <code className="text-accent">tests.json</code> → PR.
                <br />
        ...
        <section
          id="guarantee"
          className="py-24 mb-32 border-y"
          style={{
            background: 'linear-gradient(to bottom, var(--c-link-dim), var(--c-bg))',
            borderColor: 'var(--c-link-dim)',
          }}
        >
          <div className="max-w-4xl mx-auto px-6 text-center">
            <SectionLabel label="The Guarantee" gold />
            <h2 className="text-3xl md:text-4xl font-bold text-fg mb-16">
              Three things SaifCTL guarantees. <span className="text-accent">Mechanically.</span>
            </h2>
            <div className="grid md:grid-cols-3 gap-8">
              {GUARANTEES.map((g, i) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-fg text-lg mb-3">{g.title}</h3>
                  <p className="text-fg-muted text-sm leading-relaxed">{g.body}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section id="prove_it" className="max-w-6xl mx-auto px-6 mb-32">
          ...
          <div className="mt-20">
            <p className="text-fg-muted max-w-3xl leading-relaxed mx-auto">
              <strong className="text-fg">The proof is in the work:</strong>
            </p>
            <motion.div
              initial={{ opacity: 0, y: 40 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.7, delay: 0.3 }}
              className="mt-4 relative rounded-xl border border-border bg-surface overflow-hidden shadow-2xl glow-green mx-auto max-w-3xl"
            >
              {/* Terminal chrome */}
        ...
        <section id="features" className="max-w-6xl mx-auto px-6 mb-32">
          ...
          <p className="text-fg-muted max-w-2xl mb-12 leading-relaxed">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {FEATURE_CARDS.map((card, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                custom={i * 0.1}
                className="p-8 rounded-xl card-link-tint"
              >
                <div className="flex items-center gap-3 mb-4">
                  <card.Icon className="w-6 h-6 text-accent" />
                  <h3 className="font-bold text-fg text-lg">{card.title}</h3>
                </div>
                  ...
                  {card.body.map((block, j) => {
                    if (block.type === 'text') {
                      return (
                        <p
                          key={j}
                          className="text-fg-muted leading-relaxed text-sm"
                    ...
                    if (block.type === 'code') {
                      ...
                      return (
                        <div
                          key={j}
                          className="bg-bg border border-border rounded-md p-4 font-mono text-xs text-accent space-y-1"
                        >
                          {lines.map((line, k) => (
                            <div key={k}>
                    ...
                    if (block.type === 'bullets') {
                      ...
                      return (
                        <ul key={j} className="space-y-2">
                          {items.map((b, k) => (
                            <li key={k} className="flex items-start gap-2 text-sm text-fg-muted">
        ...
        <section id="deploy" className="bg-surface border-y border-border py-24 mb-32">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {DEPLOY_TIERS.map((tier, i) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-fg text-xl mb-1">{tier.name}</h3>
                  <p className="text-accent text-sm font-mono mb-4">{tier.subtitle}</p>
                  <p className="text-fg-muted text-sm leading-relaxed mb-6">{tier.body}</p>
                  {tier.openWaitlist ? (
                    <button
                      type="button"
                      onClick={openWaitlist}
                      className="text-sm text-link hover:text-link-hover hover:underline flex items-center gap-1 transition-colors"
                    >
            ...
            <p className="mt-10 text-sm text-fg-subtle text-center max-w-3xl mx-auto italic">
        ...
        <section id="security" className="max-w-6xl mx-auto px-6 mb-32">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-fg mb-4">
            ...
            <span className="text-fg-muted">So should you.</span>
          </h2>
          <p className="text-fg-muted max-w-2xl mb-4 leading-relaxed">
        ...
        <section id="reliability" className="bg-surface border-y border-border py-24 mb-32">
          <div className="max-w-6xl mx-auto px-6">
        ...
        <section id="vscode" className="max-w-6xl mx-auto px-6 mb-32">
          ...
          <div className="w-full rounded-xl border border-border bg-surface overflow-hidden mb-12 glow-green">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-surface-2">
              <span className="w-3 h-3 rounded-full bg-red-500/70" />
              <span className="w-3 h-3 rounded-full bg-yellow-500/70" />
              <span className="w-3 h-3 rounded-full bg-green-500/70" />
              <span className="ml-2 text-xs text-fg-subtle font-mono">VS Code — SaifCTL</span>
            </div>
            <div className="flex min-h-[400px]">
              {/* Activity bar */}
              <div className="w-12 bg-surface-2 border-r border-border flex flex-col items-center py-4 gap-4">
                <div className="w-6 h-6 bg-accent rounded-sm flex items-center justify-center text-bg font-bold text-xs">
                  S
                </div>
              ...
              {/* Sidebar */}
              <div className="w-64 bg-surface border-r border-border p-4 font-mono text-xs">
                <div className="text-fg-subtle uppercase text-[10px] tracking-widest mb-3">
                  SaifCTL — Features
                </div>
                <div className="space-y-2">
                  {['rate-limiting', 'auth-refresh', 'webhook-retry'].map((feat, i) => (
                    <div key={i} className="group">
                      <div className="flex items-center justify-between text-fg-muted hover:text-fg cursor-pointer py-1">
                        <span>📁 {feat}</span>
                        {i === 0 && <span className="text-accent text-[10px]">● running</span>}
                        {i === 1 && <span className="text-red-400 text-[10px]">● failed</span>}
                        {i === 2 && <span className="text-fg-subtle text-[10px]">● idle</span>}
                      </div>
                      {i === 0 && (
                        <div className="ml-4 mt-1 flex gap-2">
                          <button className="px-2 py-0.5 bg-accent/10 text-accent rounded text-[10px]">
                            ▶ Run
                          </button>
                          <button className="px-2 py-0.5 bg-border text-fg-muted rounded text-[10px]">
                            🐛 Debug
                          </button>
                        </div>
                      )}
                ...
                <div className="text-fg-subtle uppercase text-[10px] tracking-widest mt-6 mb-3">
                  Live Log — rate-limiting
                </div>
                <div className="space-y-1 text-[10px]">
                  <div className="text-fg-subtle">Attempt 1/3 — sandbox provisioned</div>
                  <div className="text-red-400">🔴 FAILED — race condition in test_concurrent</div>
                  <div className="text-fg-subtle">Attempt 2/3 — container reset</div>
                  <div className="text-accent animate-pulse">✅ PASSED — 14 tests green</div>
                </div>
              </div>
              {/* Editor placeholder */}
              <div className="flex-1 flex items-center justify-center text-fg-subtle font-mono text-sm flex-col gap-2">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {VSCODE_FEATURES.map((f, i) => (
              ...
              >
                ...
                <h3 className="font-bold text-fg mb-2">{f.title}</h3>
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              className="px-5 py-2.5 bg-accent hover:bg-accent-hover text-bg font-medium rounded-md transition-colors text-sm"
              onClick={openWaitlist}
            >
        ...
        <section id="glass-pipeline" className="bg-surface border-y border-border py-24 mb-32">
          <div className="max-w-6xl mx-auto px-6">
            <SectionLabel label="Transparency" />
            <h2 className="text-3xl md:text-4xl font-bold text-fg mb-6">
              SaifCTL shows its work. <span className="text-fg-muted">That's the point.</span>
            </h2>
            <p className="text-fg-muted max-w-2xl mb-4 leading-relaxed">
            ...
            <p className="text-fg-muted max-w-2xl mb-12 leading-relaxed">
              ...
              <strong className="text-fg">evidence</strong>. You want to know what it tried, why it
              failed, how it corrected itself, and what exactly it proved before opening a PR.
              SaifCTL attaches a full run log to every PR it opens. To prove it did the work.
            </p>

            {/* PR Mockup */}
            <div className="rounded-xl border border-border bg-surface-2 overflow-hidden mb-12 font-mono text-sm">
              {/* PR header */}
              <div className="border-b border-border p-6">
                <div className="flex items-start gap-3">
                  <GitMerge className="w-5 h-5 text-accent mt-1 shrink-0" />
                  <div>
                    <h3 className="text-fg font-bold text-base">
                      feat: implement user rate limiting
                    </h3>
                    <p className="text-fg-subtle text-xs mt-1">
                      opened by{' '}
                      <code className="text-link bg-link-dim px-1 rounded">
              ...
              {/* Run log */}
              <div className="p-6 text-fg-subtle text-xs space-y-4">
                <div>
                  <span className="text-fg font-bold">Spec Check:</span> Passed 4/4 functional
                  requirements.
                </div>
                <div>
                  <span className="text-fg font-bold block mb-2">The Factory Run Log:</span>
                ...
                <div className="border-t border-border pt-4 text-fg-subtle">
                  Agent Identity: <code className="text-link">saifctl-agent[run-a1b2c3]</code> ·
                  Runtime: 43 min · API cost: $1.42 · Full log:{' '}
                  <code className="text-accent">saifctl-run-log.md</code>
                </div>
              </div>
            </div>
        ...
        <section id="cta" className="max-w-4xl mx-auto px-6 mb-32 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 flex-wrap">
            <button
              type="button"
              onClick={openWaitlist}
              className="px-8 py-4 bg-accent hover:bg-accent-hover text-bg font-bold rounded-md transition-colors text-lg"
            >
              Join the Design Partner Waitlist
            </button>
            <a
              href={GITHUB_REPO}
              target="_blank"
              className="px-8 py-4 border border-border text-fg hover:border-link hover:text-link rounded-md transition-all text-lg"
              onClick={() => track('outbound_click', { destination: 'github_repo' })}
            >
              ★ Star on GitHub
            </a>
...
function SectionLabel({
  ...
  return (
    <div className={`flex ${center ? 'justify-center' : ''} mb-3`}>
      <span
        className={`inline-flex items-center gap-1.5 text-xs font-mono tracking-widest uppercase px-2 py-1 rounded ${gold ? 'text-accent bg-accent-dim' : 'text-fg-subtle bg-surface-2'}`}
      >
        {label}
      </span>
    </div>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border">
            <th className="text-left p-4 text-fg-subtle font-mono text-xs uppercase tracking-widest bg-surface-2 w-2/5">
              {colHeaders[0]}
            </th>
            <th className="text-left p-4 text-fg-subtle font-mono text-xs uppercase tracking-widest bg-surface-2">
              {colHeaders[1]}
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            ...
            >
              <td className="p-4 text-fg-muted align-top leading-relaxed">{row.threat}</td>
              <td className="p-4 text-fg-muted align-top leading-relaxed">{row.defense}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function RunAttempt({ n, status, detail }: { n: number; status: 'pass' | 'fail'; detail: string }) {
  return (
    <div>
      <span className={`font-bold ${status === 'pass' ? 'text-accent' : 'text-red-400'}`}>
        {status === 'pass' ? '✅' : '🔴'} Attempt {n}/3 {status === 'pass' ? 'PASSED' : 'FAILED'}
      </span>
      <p className="text-fg-subtle mt-1">{detail}</p>
    </div>
  );
}
  ...
  },
  {
    ...
    detail: (
      <>
        ...
        <br />
        <code className="text-accent">proposal.md</code> →{' '}
        <code className="text-accent">POC run</code> →{' '}
        <code className="text-accent">specification.md</code> →{' '}
        <code className="text-accent">Test files</code>
        <br />
        <br />
  ...
  },
  {
    ...
    detail: (
      <>
        ...
        <a href={LEASH_URL} target="_blank" className="text-link hover:underline">
          Leash
        </a>
  ...
  },
  {
    ...
    detail: (
      <>
        ...
        <br />
        <br />
        Configured via <code className="text-accent">gate.sh</code> in your repository.
        <br />
        <br />
  ...
  },
  {
    ...
    detail: (
      <>
        ...
        <code className="text-accent">saifctl-agent[run-id]</code>).
        <br />
        <br />
    ...
    body: [
      {
        ...
        content: (
          <>
            ...
            read
            <code className="text-accent">.env</code> files, access{' '}
            <code className="text-accent">~/.aws</code> credentials, or call external endpoints. And
            these tools ask you to trust them...
            <br />
            <br />
...
const codeCls = 'text-accent bg-accent-dim px-1 rounded';
      ...
      <>
        ...
        <ol className="list-inside mt-2 space-y-1">
          ...
          <li>
            ...
            <code className={codeCls}>.git</code> and hidden test files are never exposed to the
            agent.
          </li>
          <li>
  ...
  },
  {
    ...
    defense: (
      <>
        ...
        <a href={CEDAR_POLICY_URL} target="_blank" className="text-link hover:underline">
          Cedar
        </a>{' '}
