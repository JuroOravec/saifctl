...
local Docker images built on demand. The phases-and-critics feature has
landed on `main`; the codebase is feature-rich and the architectural
...
- **Killing the `dangerousNoLeash` mode** or the unrestricted-network
  default. Both are defensible design decisions. We adjust the *marketing
  copy* to disclose them, not the defaults.
...
| NPM-05 | B | 👍 | README has three TODO markers — `<!-- TODO - LINK TO MARKETPLACE -->` (line 87), `<!-- TODO - EMBED VIDEO OR SCREENSHOT -->` (line 88), and `[Spec-driven development](./docs/spec-driven-development.md) <!-- TODO -->` (line 114). **Decision D-10:** resolve them by *filling in*, not by removing — write `docs/spec-driven-development.md` (see DOC-08), embed real screenshots/GIF (see WEB-06 for assets), and add the marketplace link once VSX-09 + VSX publish ship. | `README.md:87-88,114` |
...
| NPM-07 | I | 👍 | Coverage thresholds at 35-40% with a self-acknowledged TODO ("require 98%"). For `v0.1`, raise the enforced floor to **70%** on critical paths (orchestrator, CLI, sandbox engine, the four kept providers from D-05). The 98% target moves to NPM-20 for `v1.0`. **Decision D-07.** | `vitest.config.ts:13-20` |
| NPM-08 | I | ➡️ | Mutable module-level state in the orchestrator loop (own TODO: "hard to reason about… hard to decouple when moving to Hatchet workflow"). Refactor into an explicit context object before declaring `1.0`. Deferred: own TODO already targets `1.0`; refactor blocks Hatchet path which is also deferred (D-04). | `src/orchestrator/loop.ts:676-681` |
| NPM-09 | B | ✅ | Remove `dist-pack/` from git history and add to `.gitignore`. The directory is a transient output for `scripts/package.sh` (local-verification path: `pnpm build && npm pack --pack-destination dist-pack/`); the *script* keeps using it, but the contents must not be tracked. **Part of Decision D-08** (repo-root dir taxonomy). Originally bundled four dirs with mixed rationale; split into NPM-09 / NPM-17 / NPM-18 / NPM-19. **Verified during tier-1 cleanup:** `dist-pack/` was already in `.gitignore` (line 4) and had nothing tracked — no action required. | `.gitignore:4`, `scripts/package.sh` |
| NPM-10 | I | ➡️ | Stray top-level planning files (`TODO_*.md`, `INBOX.md`, `plan.md`, `todo_diff_storage*.md`) are author-internal notes, already excluded from the npm tarball via the `files` array in `package.json`. Visible in the public git repo but not "shipped". Defer to `v1.0` housekeeping. | repo root |
...
| NPM-18 | B | ⚠️ | `docs_old/` is in-flight migration material to `saifdocs`. **Criterion for `v0.1`:** every concept / note / idea in `docs_old/` is transplanted to `saifdocs` (not word-for-word, conceptual coverage is the bar). After the transplant, `docs_old/` is deleted from the saifctl repo. **Part of Decision D-08.** Open: who runs the transplant audit, and what evidence proves coverage (a checklist? a reverse-link from each `docs_old/<file>` to its successor?). | `docs_old/`, `vendor/saifdocs` |
| NPM-19 | B | 👍 | `vendor/` umbrella row. Originally bundled all 5 subdirs as one item; **Decision D-12** splits this into per-subdir work items in §3.7 (VND-01..VND-05). NPM-19 stays as the cross-reference anchor; do not add new work to this row. | `vendor/`, `.gitmodules` |
| NPM-20 | I | ➡️ | Coverage uplift to **98%** across critical paths. Split out from NPM-07 by **Decision D-07** — `v0.1` lands at 70%; full uplift is `v1.0` hardening work. | `vitest.config.ts:13-20` |
| NPM-21 | I | 👍 | **README "Two modes" section** per **Decision D-20**. Insert a short section after the "SaifCTL guarantee" block (around `README.md:35`) and before "The Gauntlet" — naming **Sandbox mode** (`saifctl sandbox --agent <name>`: run any agent safely, no specs, one command) and **Factory mode** (`saifctl feat new` → `feat run`: spec-driven, gauntlet inside, phases + critics). One paragraph per mode + one CLI command per mode. Both link into the existing deep dives — Sandbox to the relocated docs at `/saifctl/sandbox/` (per WEB-08) and Factory to the existing Gauntlet section right below. | `README.md` |
...
| ID | Sev | Status | Item | Citation |
|----|-----|--------|------|----------|
| WEB-01 | B | 🟠 | Product `nav.json` files were empty stubs — the deploy workflow validates their non-emptiness. After **Decision D-20** (saifbox folds into saifctl), the saifbox nav.json drops out entirely; remaining work covers `web/src/content/docs/{saifctl,saifdocs}/nav.json`. The saifbox docs content relocates under `web/src/content/docs/saifctl/sandbox/` and its nav entries fold into the saifctl nav (tracked under WEB-08). | `web/src/content/docs/{saifctl,saifdocs}/nav.json`, `.github/workflows/publish-web.yml:56-66` |
| WEB-02 | B | 🟠 | `vendor/saifdocs` and `vendor/saifbox` are not registered submodules in `.gitmodules`, but `web/scripts/sync-docs.ts` reads from them. CI clones won't have content; `nav.json` stays empty. | `.gitmodules`, `web/scripts/sync-docs.ts` |
...
| WEB-08 | B | 👍 | **Saifbox → Sandbox-mode fold** across the web surface (per **Decision D-20**). Concrete sub-tasks: (1) `web/src/app/saifctl/page.tsx` — add a twin-hero L1 layout above the existing Factory narrative; left half = "Sandbox: run any agent safely, one command, no specs", CTA → `/saifctl/sandbox` docs; right half = "Factory: spec-driven gauntlet", CTA → scroll-to existing content. The rest of the page keeps the Factory pitch unchanged. (2) `web/src/components/Home.tsx` — drop saifbox from the `PRODUCTS` array (3→2 entries); update saifctl card copy to name both Sandbox + Factory modes in the description. (3) `web/src/components/Nav.tsx` + `Footer.tsx` — remove saifbox links (`Nav.tsx:32-33`, `Footer.tsx:49-50`). (4) `web/src/lib/docs.ts` — drop `'saifbox'` from `DOC_PRODUCTS`. (5) `web/scripts/sync-docs.ts` — drop the saifbox entry (lines 71-73). (6) `web/src/content/docs/saifbox/` → `web/src/content/docs/saifctl/sandbox/` (file moves + cross-link path rewrites `/product/saifbox/...` → `/product/saifctl/sandbox/...`). (7) `web/src/content/docs/saifctl/nav.json` — fold the relocated sandbox entries into saifctl's nav. (8) `web/src/app/saifbox/{page,layout}.tsx` — delete; add a Next.js redirect at `next.config.ts` (`/saifbox` → `/saifctl/sandbox` or `/saifctl#sandbox`). (9) `web/src/app/page.tsx:8` — update tagline to drop the "saifctl, saifbox, saifdocs" trio enumeration. | `web/src/{app,components,content,lib,scripts}/`, `web/next.config.ts` |
...
| ID | Sev | Status | Item | Citation |
|----|-----|--------|------|----------|
| DOC-01 | B | 🟠 | Top-level `README.md` links to multiple non-existent files: `docs/spec-driven-development.md`, `docs/usage.md`, `docs/guides/README.md`, `docs/features.md`, `docs/config.md`. The live content sits in `docs_old/`. Either move/regenerate into `docs/` or delete the links. | `README.md`, `docs/`, `docs_old/` |
| DOC-02 | I | ⚠️ | `/docs/` only has 5 substantive pages (`sandbox.md`, `doctor.md`, `feat-run-loop.md`, `gate-reviewer-holdout.md`, plus README). Reference docs are missing for ~20 commands (`feat new`, `feat design*`, `feat run`, all `run *` lifecycle commands except `inspect`/`sandbox`/`doctor`). | `docs/references/commands/` |
| DOC-03 | I | ⚠️ | `CONTRIBUTING.md` (45 lines) is a skeleton — no dev-loop walkthrough, branch strategy, code style, PR criteria. | `CONTRIBUTING.md` |
...
| DOC-08 | B | 👍 | Write **`docs/spec-driven-development.md`** — a step-by-step tutorial for spec-driven development the saifctl way. **Decision D-10.** Format: walk through the filesystem layout as it grows (start with an empty feature dir, add `specification.md`, then `phases/`, then `critics/`, etc.), explaining *why* each file appears. Less detailed and less opinionated than `SKILL.md` (which is the reference manual for agents); examples are short, with pointers to `SKILL.md` and the runnable `_phases-example/` for depth. Target reader: a developer evaluating saifctl who wants to know what working with it actually looks like. **Routed through DOC-09** — the page becomes a `docspec/` entry (concept, tutorial, or split between both — decided when authoring) and gets generated by saifdocs along with the rest. | `docspec/products/saifctl/{concepts,tutorials}/spec-driven-development.md` (TBD), references `SKILL.md` + `saifctl/features/_phases-example/` |
| DOC-09 | B | 👍 | **End-to-end migration of `docs/`, `docs_old/`, and `docspec/` into a single resolved + up-to-date `docs/` tree.** Consolidates DOC-01 (broken README links), DOC-02 (missing command refs), and NPM-18 (`docs_old/` transplant). See §3.4.1 for the 5-step plan and [`docs-migration.md`](safe-ai-factory/saifctl/features/release-readiness/docs-migration.md) for the per-file tracker (audit evidence). | `docs/`, `docs_old/`, `docspec/`, `README.md` |
...
| DOC-09.2 | B | 🟠 | **Migrate `docs_old/` → `docspec/` and `docs/contributing/`.** Per-file triage tracker at [`docs-migration.md`](safe-ai-factory/saifctl/features/release-readiness/docs-migration.md). Buckets: MIGRATE (user-facing → docspec), CONTRIBUTING (internal/dev → handwritten under `docs/contributing/`), DROP (obsolete). When tracker shows every row ✅, delete `docs_old/`. | `docspec/`, `docs/contributing/`, `docs_old/` (deleted) |
| DOC-09.3 | B | 🟠 | **Audit saifctl surface vs. docspec; fill gaps.** Compare CLI commands (`src/cli/commands/*.ts`), public concepts, agent profiles, env vars, etc., against the now-expanded `docspec/`. Add missing entries. Includes DOC-08 (spec-driven-development) and `feat-phases` (no docs_old counterpart). Pulls in every other "document X" item from the spec — they all become docspec entries here, BEFORE the saifdocs run in DOC-09.4. | `docspec/` (complete) |
| DOC-09.4 | B | 🟠 | **Generate.** `saifdocs gen --project-dir .` → `saifctl feat run --feature saifdocs-<ts>` → `saifdocs audit` + `saifdocs validate`. Inspect output. Hand-fix any pages where the agent output is wrong (this is exactly why we keep `docs/` git-tracked). | `docs/` (populated) |
| DOC-09.5 | I | 🟠 | **Wire CI; fix README; final cleanup.** Add `saifdocs validate` to PR CI for changes touching `src/cli/commands/`, `docspec/`, or `docs/`. Replace 17 broken `docs/` links in [`README.md`](safe-ai-factory/README.md) with regenerated paths. Update [`docs/README.md`](safe-ai-factory/docs/README.md) (if saifdocs writes one) to reflect the post-refactor compiler invocation (`saifdocs gen --project-dir .`, not the old pre-refactor flags). | `README.md`, `.github/workflows/`, `docs/README.md` |
...
- **Schema-fit findings (from `vendor/saifdocs/` source review, 2026-05-05):**
  ...
  3. **`docs/contributing/` co-location is risky**: `saifdocs clear` runs `rm -rf $outputDir` with no manifest-awareness. **Decision needed before DOC-09.2 starts** (see open decisions below).

**Resolved 2026-05-05:**

- **`docs/contributing/` location**: place at `safe-ai-factory/docs/contributing/` (co-located with generated tree). Gated on **SDR-10** — patch `saifdocs clear` to be manifest-aware (only delete files the manifest claims to own). Done up front before DOC-09.2 starts.
...
| CLM-06 | I | 🟠 | "Spec-driven AI factory" / various marketing — uses several capitalised terms (Gauntlet, Gate, Reviewer, Holdout, Critic, Reviewer-gate) that overlap. | Lock the vocabulary in a one-page glossary; the phases-and-critics plan.md already pins some of these. |
...
| VND-03 | I | 👍 | **`vendor/saifbox`** — fold into saifctl. Investigation found saifbox is literally `saifctl sandbox` (the existing CLI subcommand): every saifbox doc tutorial decodes to `saifctl sandbox --agent <name>`. saifbox was a marketing wrapper around the existing capability. **Decision D-20:** position saifctl as one product with two modes (Sandbox / Factory); twin-hero on `/saifctl` page surfaces both equally; sandbox CTA links to internal docs while the rest of the page focuses on the Factory narrative. Remaining work tracked under WEB-08 (web fold) + a small README addition (D-20 plan). After WEB-08 ships: delete `vendor/saifbox/`, drop the saifbox web page + nav entries, relocate `web/src/content/docs/saifbox/` → `web/src/content/docs/saifctl/sandbox/`, drop saifbox from `DOC_PRODUCTS`, drop the saifbox entry in `web/scripts/sync-docs.ts`, redirect `/saifbox` → `/saifctl#sandbox`. | `vendor/saifbox/`, `web/scripts/sync-docs.ts`, `web/src/app/saifbox/`, `web/src/content/docs/saifbox/` |
| VND-04 | I | ✅ | **`vendor/saifdocs`** — architectural decoupling + publish-readiness **complete 2026-05-04**. Saifdocs was refactored from an *orchestrator* (which spawned `saifctl sandbox` per-page) into a *compiler* that emits a saifctl phases-and-critics feature tree with one phase per file-to-generate. Saifdocs's runtime dependency on saifctl is gone (now devDep only); the user runs `saifctl feat run --feature saifdocs-<timestamp>` after each `saifdocs gen` to actually drive generation. **Decisions D-17** (Q-13 resolution: yes refactor; refactor done) and **D-18** (Q-12 resolution: independent publish timing). Saifdocs ships **no Cedar policy** (per Q-13's clarification — consumer repo decides). 153/153 tests passing in saifdocs; lint/typecheck/build clean. **All §3.8 SDR-01..SDR-09 closed** (SDR-08 / SDR-09: org repo migration + submodule registration; PRE-11: first publish + npm Trusted Publishing OIDC). **SDR-10** (manifest-aware `clear`) and **SDR-11** (first-class asset support) added 2026-05-05 as DOC-09 follow-ups; SDR-10 gates DOC-09.2, SDR-11 is deferred past v0.1. `@safe-ai-factory/saifdocs@0.1.0` live on npm, smoke-tested via `npx -y @safe-ai-factory/saifdocs --help`. | `vendor/saifdocs/`, `vendor/saifdocs/src/features/compiler.ts`, `web/scripts/sync-docs.ts` |
...
make saifdocs a *publishable* standalone npm package — separate from
...
| ID | Sev | Status | Item |
|----|-----|--------|------|
| X-01 | I | ⚠️ | **Smoke-test matrix.** A single `pnpm test:smoke` (or CI job) that spins up a real container, runs the simplest possible feature against the top-3 LLM providers (Anthropic, OpenAI, Google), the top-3 agentic CLIs (claude, cursor, aider), and one of each language (node-vitest, python-pytest, go-gotest, rust-rusttest). Catches integration regressions before users do. |
| X-02 | I | ⚠️ | **Cost & token observability.** No telemetry today for tokens spent / dollars per run. For a tool that loops a coder agent up to `maxRuns × testRetries × phases × critics_rounds × 2` times, this is a basic affordance. Surface in `saifctl run get` and (later) the VS Code dashboard. |
| X-03 | I | ➡️ | **Run telemetry / OTel export.** Hatchet emits workflow events; saifctl has no first-class hook to export to OTel/Sentry/Grafana. Document an env-var-driven OTel exporter at minimum. Deferred: paired with the gated Hatchet path (D-04); local-mode runs in `v0.1` produce small, inspectable logs without OTel. |
| X-04 | I | ⚠️ | **Provider SDK invocation tests** (related to NPM-06, CLM-04). After D-05 the test surface narrows to 4 native paths (Anthropic, Google, Google Vertex, OpenAI) plus one OpenAI-compatible route through `@ai-sdk/openai`. Mocked acceptance tests for each so a single SDK breaking change fails CI, not user runs. Open: live-call smoke test cadence (per-PR, nightly, or release-only?). |
| X-05 | I | ➡️ | **Indexer (`shotgun`) packaging.** README references `--indexer shotgun` (Python sub-project, `uv run python -m shotgun.main`). Distribution story unclear — does the npm install pull it? is it a separate `pip install`? Document and unify. Deferred: shotgun is opt-in (`--indexer shotgun`); for `v0.1`, document it as an optional power-user feature with a separate install step. Full packaging unification is a `v1.0` concern. |
| X-06 | N | ➡️ | **Helm chart** for the "Self-hosted and Kubernetes (Helm) deployment is underway" claim. Not blocking 0.1; blocking 1.0. |
...
harness, NPM-07's 70% coverage target on orchestrator + sandbox engine
...
| X-08-P4 | I | ➡️ | **Phase 4: coverage uplift (closes NPM-07).** Raise `vitest.config.ts` thresholds to 70/65/70/70 once P1+P2 are stable for ≥1 week. Confirm `pnpm coverage` includes integration runs. Update the TODO at `vitest.config.ts:18` to reference NPM-20 (98% v1.0 target) only. | `vitest.config.ts` |
...
**Touches:** X-08 (implementation slot), NPM-07 (unblocked by P4),
...
1. **`v0.1` coverage target: 70%** on critical paths (orchestrator,
   ...
   Tracked in `vitest.config.ts:13-20`. (NPM-07.)
2. **`v1.0` coverage target: 98%** — the original TODO. Deferred to
   the hardening cycle. (NPM-20, ➡️.)
...
- NPM-07 (70% on orchestrator + sandbox engine) is unreachable —
  those modules' "happy path" only exists in real-container runs;
  unit tests on them mock too much to count.
- X-01 (smoke-test matrix across providers × CLIs × languages) has
  nothing to compose. The matrix is a *parametrization* of harness
  invocations.
...
Touches: NPM-07 (now `v0.1`-scoped at 70%), NPM-20 (new, `v1.0`-scoped
at 98%), X-08 (the harness itself), X-01 + X-04 (now downstream of
the harness).
...
Touches: NPM-09 (narrowed), NPM-10 (deferred), NPM-17, NPM-18, NPM-19,
Q-03 (new path option (d) — vendor argus).
...
2. **Line 88 — `<!-- TODO - EMBED VIDEO OR SCREENSHOT -->`.** The
   ...
   human-only work, tracked as **PRE-01** in §8. The README only
   needs *one* hero image; the web saifctl page is where the full
   set should land.
...
adding files in the order they're needed, explaining *why* each one
...
- **CI publish:** the GitHub Actions workflow `publish-npm.yml`
  runs the same script, then publishes the *already-built tarball*:
...
- `vendor/saifdocs/src/generation/run-sandbox.ts:22-25` carries a
  *stale* comment claiming the export "is not listed in exports"
...
| `vendor/saifbox` | VND-03 | 👍 | Fold into saifctl as "Sandbox mode" (D-20). Implementation tracked under WEB-08. |
| `vendor/saifdocs` | VND-04 | ✅ | **Decoupling refactor + publish complete 2026-05-04.** Saifdocs is now a *compiler* that emits a saifctl phases-and-critics feature tree (one phase per file-to-generate) with no runtime saifctl dep. Decisions D-17 (Q-13: yes refactor) and D-18 (Q-12: independent publish). All §3.8 SDR-* items closed; `@safe-ai-factory/saifdocs@0.1.0` published via OIDC. |
...
| (saifctl-side) Phase 4 — migrate to upstream argus | VND-06 | ➡️ | Deferred until upstream PR merges *and* upstream cuts a tagged release with binaries. Concrete code changes documented in the row. |
...
upshot:** both forks are now on a glide path *toward* deletion — argus
...
### D-14 — `vendor/argus`: keep fork temporarily, fix the *actual* upstream bug, align everything
...
2. **Second re-check (corrected the corrected audit)** found that
   ...
   since v0.2.2 (2026-02-16) — *no* tagged release between v0.3.0
...
#75 merges *and* upstream cuts a tagged release that ships
...
Resolves Q-03. Locks in path (e) — the *current* wired setup — as
...
- (a) "External requirement + doctor check": this *is* what we ship
...
Saifdocs was refactored from an *orchestrator* (which spawned
`saifctl sandbox` per-page via `src/generation/run-sandbox.ts:143`)
into a *compiler*. Each `saifdocs gen` invocation now emits one
timestamped saifctl phases-and-critics feature tree under the
consumer's `saifctl/features/saifdocs-<ISO-8601-no-colons>/`, with
...
- It lets us cut RC tags (`v0.1.0-rc1`) for internal pinning without
  auto-publishing.
- Image publish is semantically a *release* event, not a ref creation.
...
- **Q-13** → resolved as **D-17** (yes, refactor saifdocs into a
  ...
  saifctl phases-and-critics feature tree per `gen` invocation).
- **Q-13 (original full body, kept for cross-references)** —
  ...
  **one timestamped feature dir** containing **N phases — one per
  ...
  pages, the result is *one* `saifctl/features/saifdocs-<timestamp>/`
...
- **Vendoring argus *source code* into saifctl's tree.** Distinct from
  shipping a *built argus binary* with saifctl's coder Docker image,
...
- **Re-architecting the orchestrator loop** — NPM-08 is "extract a
  context object", not "rewrite the loop". The loop's behavior is sound.
...
Items in this section are **not work an AI agent can complete on its
own** — they require human action, external accounts, or judgement
calls that gate downstream phase work. Capture them here so an agent
running phase work can see what's blocked on a human and not silently
stall (or worse, fabricate a stand-in artifact like a placeholder
screenshot).

Status semantics in this section:

- **🟠** — pending (human hasn't done it yet).
- **✅** — done. The human has completed the action; the blocked work
...
| PRE-07 | 🟠 | **Audit `web/x_design/` and clean it up by hand.** Walk each asset; decide what (if anything) gets kept / promoted to `web/public/` / moved to a private design folder / discarded. End state: the `web/x_design/` directory no longer exists in the working tree. Deliberately *not* gitignored — keeping it discoverable until the human resolves it prevents an agent from reaching into stale design assets later. | WEB-06 |
...
6. **`06-test-harness-and-coverage`** — net-new test infrastructure.
   ...
   provider), NPM-07 (raise threshold to 70% on critical paths), X-01
   (smoke matrix riding on the harness), X-04 (provider invocation tests
   on the four kept native paths).
   *(4 items, B+I)*
...
8. **`08-deps-and-vendor`** — the package gets healthier on disk +
   ...
   *(7 items, ⚠️-heavy until Q-11..Q-14 resolve)*
...
10. **`11-v1-deferred`** — explicit holding pen for v1.0 items, marked
    ➡️ in §3. NPM-03 (real Hatchet+resume fix; D-04 only gates the
    error message), NPM-08 (orchestrator state refactor), NPM-10
    (housekeeping for author-internal repo files), NPM-20 (98% coverage
    uplift), X-03 (OTel/Sentry export), X-05 (indexer packaging
    unification), X-06 (Helm chart), X-07 (`saifctl validate` linter),
    VND-06 (Phase 4 — migrate saifctl back to consume upstream argus
    once the upstream PR merges and a tagged release ships binaries).
    *(9 items)*
...
items NPM-07 (now `v0.1`-scoped at 70% in phase 06) and NPM-08 (now
➡️ in phase 11) split apart per Decision D-07.
...
- **A.5 saifdocs deep dive** (triggered by Q-13). Walked
  ...
  to assess refactor feasibility — could saifdocs become a *compiler*
  ...
  *(refined per user clarification)*: each saifdocs run emits **one
  ...
  snapshots). saifdocs ships *no Cedar policy*; the consumer repo
...
- The phases-and-critics feature has shipped on `main` and integrates
  with the orchestrator loop.
...
- Component prefix: 3+ uppercase letters, recognisable, mnemonic for
  ...
  | `PRE`  | Prerequisites (human-only, see §8)               |
...
**Prerequisite IDs:** `PRE-NN`, leading zero. Live in §8.
...
- **Each decision carries a Touches: line.** Lists the work-item IDs
  it affects, comma-separated. When the user asks *"what does D-04
