...
**Usage:** `saif init` (default) or `saif init --indexer shotgun`
...
The index is built automatically when you run `saif init`:
...
Re-run `saif init` whenever your codebase changes significantly. The graph ID is resolved automatically by project name — you don't need to track it.
