...
### 1. Startup script (`/factory/startup.sh`)
...
| -------------------- | --------------------------------------------- | ------------------------- |
| `factory-coder-base` | Framework only: `coder-start.sh`, `/factory/` | `docker build coder-base` |
| `factory-coder`      | Extends base; adds OpenHands                  | `docker build coder`      |

### `Dockerfile.coder-base`

- **Framework layer** — orchestration logic we always want.
- Contains only `src/orchestrator/scripts/coder-start.sh` at `/factory/coder-start.sh`.
...
- Pre-built images are on GHCR (e.g. `factory-coder-node-pnpm-python:latest`); Docker pulls automatically when not present locally.
...
- **Extensibility:** Users can `FROM factory-coder-base` and install their own coder without forking our Python/OpenHands setup.
...
/tmp/factory-sandbox/{proj}-{feat}-{runId}/
  gate.sh       ← always written; mounted :ro at /factory/gate.sh
  startup.sh    ← always written from profile or --startup-script; mounted :ro at /factory/startup.sh
  tests.full.json
...
Create `scripts/factory-startup.sh` in your repo:
...
```bash
chmod +x scripts/factory-startup.sh
```
...
Create `scripts/factory-startup.sh`:
...
This produces `factory-coder-base:latest`. (The default coder image is pulled from GHCR when omitted — extend only when bringing your own coder agent.)
...
### 1. Startup script: `scripts/factory-startup.sh`
