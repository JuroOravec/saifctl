...
async function runFactoryFloor(featureName: string) {
  ...
  while (!success && attempts < 10) {
    ...
    const openhandsCmd = runAgentOnHost
      ...
      : `node "$LEASH_BIN" --no-interactive --image saifac-coder-node-pnpm-python:latest --volume "${codePath}:/workspace" --policy policies/default.cedar /saifac/coder-start.sh`;
