...
**Usage:** `saif feat design` (default) or `saif feat design --designer shotgun`
...
saif feat design --designer shotgun --model claude-opus-4-5
...
- Shotgun manages its own codebase querying internally. When used as a designer, it does not delegate to the factory's `--indexer` tool — it runs its own research pipeline. This is why `saif init` (which builds the indexer's graph) is not required before using Shotgun as a designer.
