...
- **Install** — `agent-install.sh` runs `npm install -g @anthropic-ai/claude-code` when `claude` is missing (requires npm in the coder image). Use a Node-capable sandbox profile or bake the CLI into `--coder-image`.
