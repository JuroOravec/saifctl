...
**Usage:** `saifctl feat run --agent debug`
...
| `agent.sh`       | Writes a minimal `dummy.md` at the workspace root (`$SAIFCTL_WORKSPACE_BASE`, default `/workspace`). |

## When to use it

- End-to-end checks of the orchestration loop with a deterministic “agent” outcome.
- Developing or debugging features whose tests expect a root `dummy.md` (e.g. the bundled `dummy` saifctl feature samples).
