...
Saifctl-side environment variables — read by the **host process** (`saifctl` CLI). These are distinct from the variables passed _into_ the agent container (see `references/agent-environment.md`).
