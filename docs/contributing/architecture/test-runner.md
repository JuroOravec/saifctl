...
| `<feature>/tests/hidden/` | `/tests/hidden/` | `:ro` | Holdout specs — never staged into the agent's sandbox; mounted *only* into the test runner |
...
The test runner reaches the staging container *only* over HTTP. Two transport patterns, depending on project type.
