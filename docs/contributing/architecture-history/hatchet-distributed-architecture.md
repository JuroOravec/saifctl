...
### Phase 2 — Remote workers

Add `saifctl worker start` command. Control plane remains optional (workers can point at a
Hatchet Cloud instance or a self-hosted one).

**Deliverables:**

- `src/cli/commands/worker.ts`
- `WorkerNode` registration + heartbeat
- S3 artifact sharing between worker and control plane
- GitHub Actions worker template

### Phase 3 — Control plane server

Ship the SaifCTL API server + Next.js dashboard as a deployable service (Docker Compose /
Helm chart).

**Deliverables:**

- `server/` package: API routes, auth, project/run CRUD
- `dashboard/` package: Next.js UI (wraps Hatchet UI + SaifCTL-specific panels)
- `deploy/docker-compose.yml` for self-hosted
- `deploy/helm/` for Kubernetes

### Phase 4 — GitHub App + webhook triggers

Deploy as a GitHub App. Label an issue → run starts automatically.

**Deliverables:**

- GitHub App manifest + OAuth flow
- `POST /webhooks/github` handler
- Automatic worker provisioning (GitHub Actions pattern)
