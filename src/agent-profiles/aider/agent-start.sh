...
set -euo pipefail
trap 'ec=$?; echo "[agent-start/aider] Finished Aider setup (agent-start.sh, exit code ${ec})."' EXIT
echo "[agent-start/aider] Installing Aider (agent-start.sh)..."
...
echo "[agent-start/aider] Installing aider-chat==${AIDER_PACKAGE_VERSION} (Python ${AIDER_PYTHON_PIN}; PyPI excludes 3.13 for this release)..."

# Try different package managers
if command -v uv &>/dev/null; then
  # UV
  echo "[agent-start/aider] Installing via uv tool install..."
  ...
  if ! command -v pipx &>/dev/null; then
    ...
    if ! command -v python3 &>/dev/null; then
      echo "[agent-start/aider] ERROR: python3 is not available to bootstrap pipx." >&2
      exit 1
    fi
  ...
  if command -v pipx &>/dev/null; then
    ...
    if command -v "python${AIDER_PYTHON_PIN}" &>/dev/null; then
      echo "[agent-start/aider] Installing via pipx (python${AIDER_PYTHON_PIN})..."
      pipx install "aider-chat==${AIDER_PACKAGE_VERSION}" --python "$(command -v "python${AIDER_PYTHON_PIN}")"
    else
      echo "[agent-start/aider] ERROR: pipx needs python${AIDER_PYTHON_PIN} on PATH, or install uv." >&2
      echo "[agent-start/aider] PyPI aider-chat==${AIDER_PACKAGE_VERSION} requires Python <3.13." >&2
      exit 1
    fi
    export PATH="$HOME/.local/bin:$PATH"
    ln -sf "$(readlink -f "$HOME/.local/bin/aider")" /usr/local/bin/aider
    ...
    # pip
    if command -v "python${AIDER_PYTHON_PIN}" &>/dev/null; then
      echo "[agent-start/aider] Installing via pip (python${AIDER_PYTHON_PIN})..."
      "python${AIDER_PYTHON_PIN}" -m pip install --user "aider-chat==${AIDER_PACKAGE_VERSION}"
    else
      echo "[agent-start/aider] ERROR: need uv, pipx + python${AIDER_PYTHON_PIN}, or python${AIDER_PYTHON_PIN} for pip." >&2
      exit 1
    fi
    export PATH="$HOME/.local/bin:$PATH"
...
echo "[agent-start/aider] Aider installed: $(aider --version 2>/dev/null || echo 'unknown version')"
