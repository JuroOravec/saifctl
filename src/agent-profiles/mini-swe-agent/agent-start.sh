...
set -euo pipefail
trap 'ec=$?; echo "[agent-start/mini-swe-agent] Finished mini-SWE-agent setup (agent-start.sh, exit code ${ec})."' EXIT
echo "[agent-start/mini-swe-agent] Installing mini-SWE-agent (agent-start.sh)..."
...
echo "[agent-start/mini-swe-agent] Installing mini-swe-agent==${MINI_SWE_PACKAGE_VERSION} (Python ${MINI_SWE_PYTHON_PIN})..."

# Try different package managers
if command -v uv &>/dev/null; then
  # UV
  echo "[agent-start/mini-swe-agent] Installing via uv tool install..."
  ...
  if command -v pipx &>/dev/null; then
    ...
    if command -v "python${MINI_SWE_PYTHON_PIN}" &>/dev/null; then
      echo "[agent-start/mini-swe-agent] Installing via pipx (python${MINI_SWE_PYTHON_PIN})..."
      pipx install "mini-swe-agent==${MINI_SWE_PACKAGE_VERSION}" --python "$(command -v "python${MINI_SWE_PYTHON_PIN}")"
    else
      echo "[agent-start/mini-swe-agent] ERROR: pipx needs python${MINI_SWE_PYTHON_PIN} on PATH, or install uv." >&2
      exit 1
    fi
    export PATH="$HOME/.local/bin:$PATH"
  else
    # pip
    if command -v "python${MINI_SWE_PYTHON_PIN}" &>/dev/null; then
      echo "[agent-start/mini-swe-agent] Installing via pip (python${MINI_SWE_PYTHON_PIN})..."
      "python${MINI_SWE_PYTHON_PIN}" -m pip install --user "mini-swe-agent==${MINI_SWE_PACKAGE_VERSION}"
    else
      echo "[agent-start/mini-swe-agent] ERROR: need uv, pipx + python${MINI_SWE_PYTHON_PIN}, or python${MINI_SWE_PYTHON_PIN} for pip." >&2
      exit 1
    fi
    export PATH="$HOME/.local/bin:$PATH"
  fi
