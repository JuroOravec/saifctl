...
if command -v qwen &>/dev/null; then
  echo "[agent-install/qwen] qwen is already available: $(qwen --version 2>/dev/null || echo 'unknown version')"
  exit 0
fi
...
echo "[agent-install/qwen] Installing @qwen-code/qwen-code@${QWEN_CLI_VERSION} via npm..."
npm install -g "@qwen-code/qwen-code@${QWEN_CLI_VERSION}"
echo "[agent-install/qwen] qwen is available: $(qwen --version 2>/dev/null || echo 'unknown version')"
