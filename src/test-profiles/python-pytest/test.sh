#!/bin/sh
# Test Runner Container Entrypoint — Python / pytest runner
#
...
# Override by passing --test-script <path> to run / resume / feat:test / design-fail2pass.
#
...
#                           /tests/helpers.py    — shared test helpers imported by specs
...
# pytest -p no:cacheprovider avoids writing __pycache__ into the read-only mount.
exec pytest \
  "${SAIFAC_TESTS_DIR}" \
