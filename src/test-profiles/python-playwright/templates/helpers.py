...
Auto-generated by `saifac feat design`.
Do not edit; regenerate with `saifac feat design <featureName>`.
"""
...
def new_page() -> Page:
    """Open a Playwright browser page pointed at base_url().

    The caller is responsible for closing the browser context.
    Prefer the ``page`` pytest fixture (from pytest-playwright) in test files;
    use this helper only when you need a page outside a pytest fixture scope.
    """
