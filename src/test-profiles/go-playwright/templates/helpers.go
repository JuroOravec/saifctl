...
// Auto-generated by `saifac feat design`. Do not edit; regenerate with `saifac feat design <featureName>`.
package helpers
...
// NewPage launches a Playwright browser and returns a Page pointed at BaseURL().
// Call browser.Close() and pw.Stop() when done.
func NewPage() (playwright.Page, playwright.Browser, *playwright.Playwright, error) {
	pw, err := playwright.Run()
	if err != nil {
		return nil, nil, nil, fmt.Errorf("start playwright: %w", err)
	}
	browser, err := pw.Chromium.Launch()
	if err != nil {
		pw.Stop()
		return nil, nil, nil, fmt.Errorf("launch chromium: %w", err)
	}
	...
	if err != nil {
		browser.Close()
		pw.Stop()
		return nil, nil, nil, fmt.Errorf("new page: %w", err)
	}
